...
VST_clone_table_SET<- unique(VST_clone_table$cdr3) 
# uniq cdr3
VST_clone_table_uniq<-VST_clone_table[!duplicated(VST_clone_table[,c('cdr3')]),]
...
#   geom_bar(stat = 'identity') 
s1_clone_table_SET<- unique(s1_clone_table$cdr3) 

#vs vst
s1_clone_table<-s1_clone_table[!duplicated(s1_clone_table[,c('cdr3')]),]
csr3_s1<- as.vector(VST_clone_table_uniq$cdr3) 
vst_s1<- subset(s1_clone_table, cdr3 %in% csr3_s1)
vst_s1$clone<-NULL
vst_merge<-subset(VST_clone_table_uniq,select=c("cdr3","clone","clone_size"))
vst_s1<-merge(vst_s1,vst_merge, by="cdr3",suffixes = c(".S1",".VST"))
...
#   geom_bar(stat = 'identity') 
s2_clone_table_SET<- unique(s2_clone_table$cdr3)

#vs vst
s2_clone_table<-s2_clone_table[!duplicated(s2_clone_table[,c('cdr3')]),]
csr3_s2<- as.vector(VST_clone_table_uniq$cdr3) 
vst_s2<- subset(s2_clone_table, cdr3 %in% csr3_s2)
vst_s2$clone<-NULL
vst_merge<-subset(VST_clone_table_uniq,select=c("cdr3","clone","clone_size"))
vst_s2<-merge(vst_s2,vst_merge, by="cdr3",suffixes = c(".S2",".VST"))
...
#   geom_bar(stat = 'identity') 
s3_clone_table_SET<- unique(s3_clone_table$cdr3) 

#vs vst
s3_clone_table<-s3_clone_table[!duplicated(s3_clone_table[,c('cdr3')]),]
csr3_s3<- as.vector(VST_clone_table_uniq$cdr3) 
vst_s3<- subset(s3_clone_table, cdr3 %in% csr3_s3)
vst_s3$clone<-NULL
vst_merge<-subset(VST_clone_table_uniq,select=c("cdr3","clone","clone_size"))
vst_s3<-merge(vst_s3,vst_merge, by="cdr3",suffixes = c(".S3",".VST"))
...
#   geom_bar(stat = 'identity') 
s4_clone_table_SET<- unique(s4_clone_table$cdr3) 

#vs vst
s4_clone_table<-s4_clone_table[!duplicated(s4_clone_table[,c('cdr3')]),]
csr3_s4<- as.vector(VST_clone_table_uniq$cdr3) 
vst_s4<- subset(s4_clone_table, cdr3 %in% csr3_s4)
vst_s4$clone<-NULL
vst_merge<-subset(VST_clone_table_uniq,select=c("cdr3","clone","clone_size"))
vst_s4<-merge(vst_s4,vst_merge, by="cdr3",suffixes = c(".S4",".VST"))
vst_s4<- vst_s4 %>% relocate("clone", .before = "clone_size.S4")

all_apearing_clones<- append(vst_s1$cdr3,vst_s2$cdr3)
all_apearing_clones<- append(all_apearing_clones,vst_s3$cdr3)
all_apearing_clones<- append(all_apearing_clones,vst_s4$cdr3)
all_apearing_clones<- unique(all_apearing_clones)
VST_apearing<-subset(VST_clone_table_uniq, cdr3 %in% all_apearing_clones)

#all_overlap_clones
common_clones_list <- c(unique(VST_clone_table_uniq$cdr3),unique(vst_s1$cdr3),unique(vst_s2$cdr3),unique(vst_s3$cdr3),unique(vst_s4$cdr3))
vst_all_overlap <-intersect(intersect(intersect(intersect(VST_clone_table_uniq$cdr3,vst_s1$cdr3),vst_s2$cdr3),vst_s3$cdr3),vst_s4$cdr3)
VST__all_overlap_tb<-subset(VST_clone_table_uniq, cdr3 %in% vst_all_overlap)
vst_s1_sub<-subset(vst_s1, cdr3 %in% vst_all_overlap, select = c("cdr3","clone","clone_size.S1"))
vst_s2_sub<-subset(vst_s2, cdr3 %in% vst_all_overlap, select = c("cdr3","clone","clone_size.S2"))
vst_s3_sub<-subset(vst_s3, cdr3 %in% vst_all_overlap, select = c("cdr3","clone","clone_size.S3"))
vst_s4_sub<-subset(vst_s4, cdr3 %in% vst_all_overlap, select = c("cdr3","clone","clone_size.S4"))
...
df_list_tb<- df_list %>% reduce(full_join, by='cdr3')
...
care_t_name<-args[1]
outname <- paste0(care_t_name,"_overlap_clones(10+)_by_cdr3-Report_new.xlsx")
...
png_name<-paste0(care_t_name,"_ven_by_cdr3_(clone_size_10+).png")
