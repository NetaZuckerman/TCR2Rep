...
#input1$VST_45F <-NULL
input1t = t(input1)
rownames(input1t)<-c("Car_T","2022-09-29", "2022-10-26", "2022-11-17", "2022-12-13")
...
#input2$`45F_S5` <-NULL
input2t = t(input2)
rownames(input2t)<-c("Car_T","2022-09-29", "2022-10-26", "2022-11-17", "2022-12-13")
...
#input2$`45F_S5` <-NULL
input3t= t(input3)
rownames(input3t)<-c("Car_T","2022-09-29", "2022-10-26", "2022-11-17", "2022-12-13")
#rownames(input2)<-  format(as.Date(rownames(input2)), "%Y/%m/%d")
...
ggplot() +
  geom_line(data3, mapping=aes(x = Var1 ,y = value, color = New_Clones, group = Var2)) +
  scale_colour_manual(values = "grey85", labels ="")+
  new_scale_color() +
  ...
  labs(
    caption = "New_Clones- are clones that not exist in the given CAT-T cells.\nInconsist_Clones - are clones that appear in treatment but not in all samples. \nConsistent Clones - are clones that are shared by all samples",
