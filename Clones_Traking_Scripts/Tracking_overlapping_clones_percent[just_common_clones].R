...
#new - TRACK JUST AFTER COMMON CLONE 
common_clone<- rbind(A1_comm_T,A2_comm_T ,A3_comm_T,A4_comm_T)
common_clone$clone<-NULL
common_clone$clone_size <-NULL
common_clone<-common_clone %>% count("junction")%>% filter(freq == "4")
#CHOOSE JUST JUNCTIONS/CLONES THAT APEAR in CARE common_clone
A1_comm_T <- A1_jun[A1_jun$junction %in% common_clone$junction,]
A2_comm_T <- A2_jun[A2_jun$junction %in% common_clone$junction,]
A3_comm_T <- A3_jun[A3_jun$junction %in% common_clone$junction,]
A4_comm_T <- A4_jun[A4_jun$junction %in% common_clone$junction,]
T_jun <- T_jun[T_jun$junction %in% common_clone$junction,]
...
junction_AA<- subset(T_care,productive=='TRUE' & cdr3!= 'NULL' & junction!='NULL',select=c("junction","junction_aa"))
junction_AA <- junction_AA[!duplicated(junction_AA[ , "junction"]), ]
junction_AA <- junction_AA[junction_AA$junction %in% common_clone$junction,]
...
outname <- paste0(path,table_name,"_Clones_Tracking_[percent]290123_NETA.xlsx")
print(outname)
